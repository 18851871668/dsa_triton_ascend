"""Torch helpers for sfa_core_standalone._sfa_core — CPU-generate variant.

Inputs are generated on CPU (no NPU memory used for random data). Callers move
tensors to NPU via to_device() before run_sfa(). run_sfa infers buffer device
from q.device.
...
def make_inputs(B, S1, S2, N1, D, dtype=torch.float16, seed=42):
    rng = np.random.RandomState(seed)
    q = _np(rng.randn(B, S1, N1, D), dtype)
    k = _np(rng.randn(B, S2, 1, D), dtype)
    qr = _np(rng.randn(B, S1, N1, D_ROPE), dtype)
    kr = _np(rng.randn(B, S2, 1, D_ROPE), dtype)
    return q, k, qr, kr
